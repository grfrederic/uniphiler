...
topDef(def(Id, RetType, Args, Body, _Loc)) -->
    { function_context_and_regs_init(Args, Cont, Regs),
      blck_builder(Body, Cont, _ContNext, LlvmGlobals, LlvmBlocks_),
      maplist(add_missing_terminals(RetType), LlvmBlocks_, LlvmBlocks)
    }, !,
    LlvmGlobals, [func(RetType, Id, Regs, LlvmBlocks)].
...
add_missing_terminals(
  RetType,
  llvmBlock(Label, LlvmLines_),
  llvmBlock(Label, LlvmLines)
) :-
  (   last_is_terminal(LlvmLines_), !
  ->  LlvmLines_ = LlvmLines
  ;   (   RetType = void
      ->  append(LlvmLines_, [return], LlvmLines)
      ;   append(LlvmLines_, [unreachable], LlvmLines)
      )
  ), !.

last_is_terminal(LlvmLines) :-
    append(_, [Last], LlvmLines),
    trm_(Last),
    !.


% === BLOCK BUILDER ===
% these are used to refine the raw lines generated by
% DCG below and pack them into global definitions / llvm blocks

blck_builder(Body, Cont, ContNext, LlvmGlobals, LlvmBlocks) :-
    phrase(blck(Body, Cont, ContNext), LlvmLines), !,
    builder(LlvmLines, LlvmGlobals, LlvmBlocks).


builder(LlvmLines, LlvmGlobals, LlvmBlocks) :-
    foldl(bld_, LlvmLines,
          ([], [llvmBlock(_EntryBlock, [])]),
          (LlvmGlobalsRev, LlvmBlocksRevRev)),
    reverse(LlvmGlobalsRev, LlvmGlobals),
    reverse(LlvmBlocksRevRev, LlvmBlocksRev),
    maplist(revblck_, LlvmBlocksRev, LlvmBlocks).


revblck_(
  llvmBlock(Label, LlvmLinesRev),
  llvmBlock(Label, LlvmLines)
) :-
    reverse(LlvmLinesRev, LlvmLines).


bld_(
  constant(C, T, V),
  (LlvmGlobals, LlvmBlocks),
  ([constant(C, T, V)|LlvmGlobals], LlvmBlocks)
) :- !.

bld_(
  label(Label),
  (LlvmGlobals, [llvmBlock(_OldLabel, LlvmLines)|LlvmBlocks]),
  (LlvmGlobals, [llvmBlock(Label, LlvmLines)|LlvmBlocks])
) :- !.

bld_(
  LlvmLine,
  (LlvmGlobals, [llvmBlock(Label, LlvmLines)|LlvmBlocks]),
  (LlvmGlobals, LlvmBlocksNew)
) :-
    (   trm_(LlvmLine)
    ->  LlvmBlocksNew = [llvmBlock(_FreshBlock, []), llvmBlock(Label, [LlvmLine|LlvmLines])|LlvmBlocks]
    ;   LlvmBlocksNew = [llvmBlock(Label, [LlvmLine|LlvmLines])|LlvmBlocks]
    ), !.
